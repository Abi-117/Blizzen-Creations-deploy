...
export default function LandingPage() {
  ...
  useEffect(() => {
    const fetchLandingData = async () => {
      ...
      setError(null);
      try {
        const response = await apiFetch("/landing");
        setData(response);
      } catch (err: any) {
        console.error("Failed to fetch landing data:", err);
        setError("Unable to load landing page data. Please try again later.");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!data) return <div className="text-center p-8 text-red-600">No landing data available.</div>;

  return (
    <main className="overflow-hidden">
      {/* HERO */}
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20 bg-gradient-to-b from-primary/10 to-background">
        <div className="container-custom relative z-10 px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-12">
            ...
            >
              <a href="#contact">
                {data.hero.cta || "Enroll Now"} <ArrowRight className="ml-2 w-5 h-5" />
              </a>
            </Button>

...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrentIndex(
        (prev) => (prev + 1) % data.courses.length
      );
    }, 4000);
  ...
  const handleApplyNow = (course: string) => {
    ...
    window.open(
      `https://wa.me/919884264816?text=${msg}`,
      "_blank"
    );
  };
  ...
  return (
    <main className="overflow-hidden">
      ...
      {/* ABOUT */}
      <section id="about" className="p-10 bg-muted/30">
        <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-10">
          ...
          <div className="grid grid-cols-2 gap-4">
            {highlights.map((h, i) => (
              <div
                key={i}
                className="p-5 bg-background border rounded-lg"
              >
      ...
      {data.courses.length > 0 && (
        <section id="courses" className="p-10">
      ...
      {/* CONTACT */}
      <section id="contact" className="p-10 bg-muted/20">
